...
| **Privacy-preserving visitors**   | Derives a site-scoped HMAC from the UTC day and short-lived request signals at the Worker, then counts distinct daily keys for totals, pages, referrers, countries, browsers, and devices. Raw IP addresses are never stored, and keys cannot correlate people across sites or days. |
...
**No tracking plugin, no cookies, and no persistent cross-day identifier.** A
secret HMAC produces a non-reversible visitor key scoped to one site and one
UTC day. The Worker stores only that rotating key, never the source IP address.
Click IDs are stored by name (`gclid`, for example), never by value, because
the value identifies an individual click.
